...
type jobParameters struct {
	...
	LanguageCode    Language `json:"language_code,omitempty"`
...
func (s *jobStatus) job() *Job {
	...
	for _, d := range s.JobDetails {
		f := JobFile{State: d.State, Error: d.ErrorMessage}
		if len(d.Inputs) > 0 {
...
func (b *BatchService) Create(ctx context.Context, req *BatchRequest) (*Job, error) {
	...
	byName := make(map[string]Input, len(req.Files))
...
func uniqueName(name string, taken map[string]Input) string {
	if name == "" {
		name = "audio"
	}
...
func (b *BatchService) upload(ctx context.Context, target string, in Input, storage string) error {
...
func uploadBody(in Input) (io.ReadCloser, int64, error) {
...
func (b *BatchService) Get(ctx context.Context, jobID string) (*Job, error) {
	if jobID == "" {
...
func (b *BatchService) Wait(ctx context.Context, jobID string, poll time.Duration) (*Job, error) {
	if poll <= 0 {
		poll = batchPollWait
	}
	for {
...
func (b *BatchService) Results(ctx context.Context, job *Job) (map[string]*TranscriptionResult, error) {
	if job == nil || job.ID == "" {
	...
	results := make(map[string]*TranscriptionResult, len(outputs))
	if len(outputs) == 0 {
		return results, nil
	}
...
func (b *BatchService) download(ctx context.Context, target string) (*TranscriptionResult, error) {
